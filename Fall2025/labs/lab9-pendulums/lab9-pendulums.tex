...
Use a photogate to help you measure the period.
...
Graph your data and state your conclusions about whether $T$ depends on $\theta_{\rm{max}}$, $L$ and $m$.  Remember that on a graph of experimental data, the horizontal axis should always be the quantity that you controlled directly (in this case $\theta_{\rm{max}}$, $L$, or $m$) and the vertical axis should be the quantity that you measured but did not directly select (in this case $T$). In order to compare the graphs, make sure that the scale (both x- and y-axes) are identical on all plots. %You should consider (and try to minimize) your sources of error.  There may be some fairly significant systematic errors.
...
HINT: The pendulum's period should follow a power law relationship with the variable that most strongly impacts the period. In order to determine the relationship between the period and that variable, take the logarithm of both the period and the variable and plot them in a linear graph. The data points should fall on a straight line, and the slope and y-intercept of the line allow you to determine the form of the power law relationship (as we have done several times throughout the semester). What is the meaning of the coefficients that you found? Think about the units to help you figure it out.
