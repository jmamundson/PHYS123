...
\sol{
There are a couple of different ways you could show this. In my sketch I subtracted the initial energy from each of the terms, so that all of the plots start at 0. The elastic potential and gravitational potential energies should be out of phase with each other, and the kinetic energy should oscillate twice as fast. The kinetic energy reaches a maximum twice during each oscillation: once as the spring is contracting and once as it is extending.
\begin{center}
...
\clearpage
5. The figure below is a snapshot of a transverse traveling wave at $t=0$~s. The wave is traveling to the left at 1~m/s. Write down the wave model for this wave. [4 pts]

\begin{center}
\includegraphics[width=12cm]{./exam3_5.eps}
\end{center}

\sol{
The amplitude and wavelength of the wave can be directly seen from the graph.
$$A=2\mbox{ cm}$$
$$\lambda = 3\mbox{m}$$
The wave speed is given, which means that the period can be calculated using
$$v=\frac{\lambda}{T}$$
$$T=\frac{\lambda}{v}=3\mbox{ s}$$
Therefore the wave models is
$$\boxed{\Delta y(x,t) = 2\cos\left(2\pi\frac{x}{3}+2\pi\frac{t}{3}\right)}$$
