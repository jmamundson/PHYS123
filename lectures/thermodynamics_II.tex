...
\subsubsection*{Example \#1: Melting snowpack}
...
\subsubsection*{Example \#2: Place hot metal in water}
...
\subsubsection*{Example \#3: Place ice in water}
...
What if instead we had started with 300 g of water at 20$^\circ$C? This means that the amount of heat that the water can lose is $m_wc_w\Delta{T_w}=25200\mbox{ J}$, which is more than enough to melt all of the ice. We can set up an equation then that looks like
$$m_ic_i\Delta T_i + m_iL_f + m_ic_w(T_f-T_0) + m_wc_w(T_f-T_w) = 0,$$
where $T_f$ is the final temperature of the system and $T_0=273$~K. Solving for the final temperature,
$$T_f = \frac{m_i(c_wT_0-L_f-c_i\Delta T_i) + m_wc_wT_w}{(m_i+m_w)c_w}=280\mbox{ K}=7^\circ\mbox{C}$$


\subsection{Bomb calorimeters}
Sensible heat is used to determine the amount of energy stored in food.
...
\subsection{Energy consumption by swimming seals}
Animals that swim have to overcome frictional forces from the water. Due to buoyancy of the water, they do not have to work against gravity. Walking or running on land is fundamentally different since air resistance is relatively small compared to drag from water, but walking and running also involve small but frequent changes in elevation (i.e., gravitational potential energy).

The drag force acting on a swimming animal can be expressed as
$$F_d = \frac{1}{2}C_d\rho_wAv^2$$
For a harbor seal, $C_d\approx 0.1$ and $A\approx 0.1\mbox{ m}^2$. Harbor seals are most efficient at speeds of about 1.5~m/s and are often observed swimming at these speeds. For a seal to swim 10~km, it must do an amount of work equal to $F_d\Delta x= 112500\mbox{ J} = 27\mbox{ kcal}$. Assuming 25\% efficiency implies that the will burn 107~kcal, which is a small fraction of the roughly 6000 kcal that they consume every day.
