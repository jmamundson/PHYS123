...
\item Pressure = force / area; depends on weight of overlying fluid
  \begin{itemize}
  \item For liquids in hydrostatic equilibrium: $P=P_{0}+\rho gh$, where $P_0$ is the pressure at the top of the liquid (could be atmospherice pressure or gas pressure)
...
\item The fluid flow is steady (laminar) and non-turbulent.
  ...
  [Sketch of laminar and turbulent flow.]
  \vspace{3cm}
  ...
  [Show video of water coming from faucet.]
...
The incompressibility assumption leads to a nice continuity equation, which is really a statement of mass conservation, for fluid flow through a \textit{pipe}:
$$Q=vA=\mbox{constant}$$
...
Note that the continuity equation is different for open channel flow, because the depth of the channel can also change. In that case,
...
The flux continuity equation tells us that fluid flow is constant through straight stretches of pipe, but accelerates or decelerates as the pipe diameter varies.
...
To understand how pressure causes fluid flow, we'll use the work-energy theorem.
...
[Diagram of pipe used to derive Bernoulli's equation.]\nopagebreak
\vspace{5cm}
...
[Diagram of a pipe with various diameters and elevations.]\nopagebreak
\vspace{8cm}
