...
We'll spend the next few lectures discussing the motion of fluids by using the ideas of forces and energy.
...
\subsection{Density, pressure, and hydrostatic equilibrium}
[Demo of balloons floating over dry ice.]
...
[Switch to slides that describe differences between liquid pressure and gas pressure.]
...
[Diagram with a parcel of water in a container.]
\vspace{5cm}

Let's assume that the parcel is in hydrostatic equilibrium, meaning that it's not flowing.
...
An object that is more dense than water will sink, and the rate at which it sinks depends on the density differences. (If this were an object falling through air, $\rho_o>>\rho-w$ and so $a_y=-g$.0
...
Consider an iceberg floating at rest at the water surface. 
...
In a manometer, this means that 
$$P_1=P_2$$
where
$$P_1=P_{gas}$$
and
...
Mercury has a density of 13,534 kg/m$^3$. The column of mercury that you need is only 0.76 m tall --- this is about 30 in of mercury. For really accurate pressure readings, you need to account for thermal expansion of mercury, for which you need a thermometer.


\subsubsection*{Demo of pipette holding water}
Use a pipette or straw to illustrate water pressure in a vacuum
$$P_0=\rho g d + P_{bubble}$$
$$P_{bubble} = P_0-\rho g d$$
If $d=10$~cm and $\rho=1000$~kg/m$^3$, then $P_{bubble}=100.344$~kPa

What is the maximum height straw that you could conceivably use?
